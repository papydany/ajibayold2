...
$student = array();
      ...
      $query ="SELECT DISTINCT sr.student_id,sp.class_option_id,sp.surname,sp.firstname,sp.othername,sp.student_no,sp.gender FROM student_profile as sp LEFT JOIN student_reg as sr ON sp.student_id = sr.student_id   WHERE sr.class_id ='$class_id' && sr.class_category_id ='$class_category_id' && sr.session='$s' && sp.status='present' order by sp.student_no";
      $result = mysqli_query($conn,$query) or die (mysqli_error($conn));
      if(!$result){
            echo " query faild select";
      }
